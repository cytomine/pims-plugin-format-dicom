...
class WsiDicom:
    ...
    def read_region(
        ...
        print(f"found level: {wsi_level}")
        scale_factor = wsi_level.calculate_scale(level)
        
        if location[0]*scale_factor + size[0]*scale_factor >= wsi_level.size.width:
            limit_width = wsi_level.size.width - location[0]*scale_factor
        else:
            limit_width = size[0]*scale_factor
            
        if location[1]*scale_factor + size[1]*scale_factor >= wsi_level.size.height:
            limit_height = wsi_level.size.height - location[1]*scale_factor
        else:
            limit_height = size[1]*scale_factor

        position = Point.from_tuple(location)*scale_factor
        scaled_region = Region(
            position,
            size=Size.from_tuple((limit_width, limit_height))
        )
    ...
    def _get_filepaths(path: Union[str, List[str], Path, List[Path]]):
        ...
        if isinstance(path, (str, Path)):
            single_path = Path(path)
            print(single_path)
            print(repr(single_path))
            print(os.path.isdir(single_path))
            if single_path.is_dir():
                print("if1")
                return list(single_path.iterdir())
            elif single_path.is_file():
                if os.path.splitext(single_path)[1] == ".zip":
